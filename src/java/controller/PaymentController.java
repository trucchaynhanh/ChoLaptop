...
public class PaymentController extends HttpServlet {
    ...
    private void checkOut(HttpServletRequest request, HttpServletResponse response) {
        ...
        Order cart = (Order) session.getAttribute("cart");
        //lay ve account id
        int accountId = ( (Account) session.getAttribute(commonConstant.SESSION_ACCOUNT)).getId();
        List<Products> list = (List<Products>) session.getAttribute(commonConstant.SESSION_PRODUCT);
        //amount
    ...
    for (OrderDetails od : cart.getListOrderDetails()) {
        Products product = productsDAO.getById(od.getProductId());
    ...
    private int calculateAmount(Order cart, List<Products> list) {
        int amount = 0;
        for (OrderDetails od : cart.getListOrderDetails()) {
    ...
    private float findPriceById(List<Products> list, int bookId) {
